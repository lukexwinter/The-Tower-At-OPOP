<?php include($_SERVER['DOCUMENT_ROOT']."/the-tower-at-opop/live-site/includes/constants.php"); ?>
<div class="floorplan">
	<div class="fp-image">
		<img src="<?php echo MAINURL."live-site/img/03-floorplan.png"; ?>" alt="" />
	</div>
	<div class="fp-info">
		<h1>03</h1>
		<p>
			Studio<br />
			Floors 3-23
		</p>
		<div style="clear: both;"></div>
		<img src="<?php echo MAINURL."live-site/img/03-floorplan.png"; ?>" alt="" />
		<p class="starting-price">Starting at $1,195</p>
		<a href="#" class="button desktop">View Availability</a>
		<ul class="clearfix">
			<li>One Bedroom</li>
			<li>One Bathroom</li>
			<li>761 sq ft</li>
			<li><a href="<?php echo MAINURL."live-site/pdf/03.pdf"; ?>" target="_blank">Download Floor Plan</a></li>
		</ul>
		<a href="#" class="button mobile">View Availability</a>
	</div>
</div>
<div class="availability">
	
	<h1>03 Availability</h1>
	<div class="table-wrapper">
		<table class="available-apartments" cellspacing="0">
		   <thead>
		      <tr>
		         <th class="persist essential">Unit #</th>
		         <th class="optional">Floor</th>
		         <th class="optional">Balcony</th>
		         <th class="essential">Monthly Price</th>
		         <th class="essential" style="width: 20px;"></th>
		      </tr>
		   </thead>
		   <tbody>
		      <tr class="reserved">
		         <td>2303</td>
		         <td>23</td>
		         <td>Yes</td>
		         <td>$1,455.00</td>
		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>
		      </tr>      
		      <tr>
		         <td>2203</td>
		         <td>22</td>
		         <td>Yes</td>
		         <td>$1,425.00</td>
		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>
		      </tr>     
		      <tr>
 		         <td>2103</td>
 		         <td>21</td>
 		         <td>Yes</td>
 		         <td>$1,425.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>
		      </tr>       
		      <tr>
 		         <td>2003</td>
 		         <td>20</td>
 		         <td>Yes</td>
 		         <td>$1,425.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>
		      </tr>      
		      <tr>
 		         <td>1903</td>
 		         <td>19</td>
 		         <td>Yes</td>
 		         <td>$1,425.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>
		      </tr>
		      <tr>
 		         <td>1803</td>
 		         <td>18</td>
 		         <td>Yes</td>
 		         <td>$1,425.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>
		      </tr>      
		      <tr>
 		         <td>1703</td>
 		         <td>17</td>
 		         <td>Yes</td>
 		         <td>$1,355.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1603</td>
 		         <td>16</td>
 		         <td>Yes</td>
 		         <td>$1,355.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1503</td>
 		         <td>15</td>
 		         <td>Yes</td>
 		         <td>$1,355.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1403</td>
 		         <td>14</td>
 		         <td>Yes</td>
 		         <td>$1,355.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1303</td>
 		         <td>13</td>
 		         <td>Yes</td>
 		         <td>$1,355.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1203</td>
 		         <td>12</td>
 		         <td>Yes</td>
 		         <td>$1,295.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1103</td>
 		         <td>11</td>
 		         <td>Yes</td>
 		         <td>$1,295.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1003</td>
 		         <td>10</td>
 		         <td>Yes</td>
 		         <td>$1,295.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>903</td>
 		         <td>9</td>
 		         <td>Yes</td>
 		         <td>$1,295.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>803</td>
 		         <td>8</td>
 		         <td>Yes</td>
 		         <td>$1,295.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>703</td>
 		         <td>7</td>
 		         <td>Yes</td>
 		         <td>$1,230.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>603</td>
 		         <td>6</td>
 		         <td>Yes</td>
 		         <td>$1,230.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>503</td>
 		         <td>5</td>
 		         <td>Yes</td>
 		         <td>$1,230.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>403</td>
 		         <td>4</td>
 		         <td>No</td>
 		         <td>$1,195.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>303</td>
 		         <td>3</td>
 		         <td>No</td>
 		         <td>$1,195.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		   </tbody>
		</table>
		<small style="text-align: left; color: lightgray; font-size: 11px; margin-top: 25px;">NOTE:<br />Floors 3 &amp; 4 are without balcony</small>
		
	</div><!-- /table-wrapper -->
</div>
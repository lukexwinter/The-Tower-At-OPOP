<?php include($_SERVER['DOCUMENT_ROOT']."/the-tower-at-opop/live-site/includes/constants.php"); ?>
<div class="floorplan">
	<div class="fp-image">
		<img src="<?php echo MAINURL."live-site/img/02-floorplan.png"; ?>" alt="" />
	</div>
	<div class="fp-info">
		<h1>02</h1>
		<p>
			One Bedroom<br />
			Floors 3-23
		</p>
		<div style="clear: both;"></div>
		<img src="<?php echo MAINURL."live-site/img/02-floorplan.png"; ?>" alt="" />
		<p class="starting-price">Starting at $1,295</p>
		<a href="#" class="button desktop">View Availability</a>
		<ul class="clearfix">
			<li>One Bedroom</li>
			<li>One Bathroom</li>
			<li>801 sq ft</li>
			<li><a href="<?php echo MAINURL."live-site/pdf/02.pdf"; ?>" target="_blank">Download Floor Plan</a></li>
		</ul>
		<a href="#" class="button mobile">View Availability</a>
	</div>
</div>
<div class="availability">
	
	<h1>02 Availability</h1>
	<div class="table-wrapper">
		<table class="available-apartments" cellspacing="0">
		   <thead>
		      <tr>
		         <th class="persist essential">Unit #</th>
		         <th class="optional">Floor</th>
		         <th class="optional">Balcony</th>
		         <th class="essential">Monthly Price</th>
		         <th class="essential" style="width: 20px;"></th>
		      </tr>
		   </thead>
		   <tbody>
		      <tr class="reserved">
		         <td>2301</td>
		         <td>23</td>
		         <td>Yes</td>
		         <td>$1,555.00</td>
		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>
		      </tr>      
		      <tr>
		         <td>2201</td>
		         <td>22</td>
		         <td>Yes</td>
		         <td>$1,525.00</td>
		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>
		      </tr>     
		      <tr>
 		         <td>2101</td>
 		         <td>21</td>
 		         <td>Yes</td>
 		         <td>$1,525.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>
		      </tr>       
		      <tr>
 		         <td>2001</td>
 		         <td>20</td>
 		         <td>Yes</td>
 		         <td>$1,525.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>
		      </tr>      
		      <tr>
 		         <td>1901</td>
 		         <td>19</td>
 		         <td>Yes</td>
 		         <td>$1,525.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>
		      </tr>
		      <tr>
 		         <td>1801</td>
 		         <td>18</td>
 		         <td>Yes</td>
 		         <td>$1,525.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>
		      </tr>      
		      <tr>
 		         <td>1701</td>
 		         <td>17</td>
 		         <td>Yes</td>
 		         <td>$1,455.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1601</td>
 		         <td>16</td>
 		         <td>Yes</td>
 		         <td>$1,455.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1501</td>
 		         <td>15</td>
 		         <td>Yes</td>
 		         <td>$1,455.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1401</td>
 		         <td>14</td>
 		         <td>Yes</td>
 		         <td>$1,455.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1301</td>
 		         <td>13</td>
 		         <td>Yes</td>
 		         <td>$1,455.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1201</td>
 		         <td>12</td>
 		         <td>Yes</td>
 		         <td>$1,395.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1101</td>
 		         <td>11</td>
 		         <td>Yes</td>
 		         <td>$1,395.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>1001</td>
 		         <td>10</td>
 		         <td>Yes</td>
 		         <td>$1,395.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>901</td>
 		         <td>9</td>
 		         <td>Yes</td>
 		         <td>$1,395.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>801</td>
 		         <td>8</td>
 		         <td>Yes</td>
 		         <td>$1,395.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>701</td>
 		         <td>7</td>
 		         <td>Yes</td>
 		         <td>$1,330.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>601</td>
 		         <td>6</td>
 		         <td>Yes</td>
 		         <td>$1,330.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>501</td>
 		         <td>5</td>
 		         <td>Yes</td>
 		         <td>$1,330.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>401</td>
 		         <td>4</td>
 		         <td>No</td>
 		         <td>$1,295.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		      <tr>
 		         <td>301</td>
 		         <td>3</td>
 		         <td>No</td>
 		         <td>$1,295.00</td>
 		         <td><a class="button" href="">Apply</a><span class="reserved">reserved</span></td>       
		      </tr>
		   </tbody>
		</table>
		<small style="text-align: left; color: lightgray; font-size: 11px; margin-top: 25px;">NOTE:<br />Floors 3 &amp; 4 are without balcony</small>
		
	</div><!-- /table-wrapper -->
</div>
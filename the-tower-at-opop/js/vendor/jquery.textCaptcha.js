	...
	$.fn.textCaptcha = function(options) {
		...
		return this.each(function() {
			...
			switch(options.method) {
				...
				// math
				case 'add':
					captcha_method_text = '<em style="font-size: 10px; display: block;">(Security Question)</em> Add these numbers';
				break;
			...
			function output(el) {
				...
				// $('div[id^="list"]');
				var output = '<div class="formline"> <label style="white-space:nowrap;"><span id="type_captcha_method_'+ identifier + '">'+captcha_method_text+'</span>: <span id="captcha">'+captcha_string+'</span><span class="redtext"> *</span></label>';
				output += '<input type="text" id="captcha_user_'+ identifier + '" name="captcha_user" class="textbox" /></div>';
				...
				el.find('.submitline').before(output);
			}
